...
import { sendBatchReadyEmail } from '@/lib/emailService'; // ✅ ADD EMAIL IMPORT
...
export default function BatchActionModal({ open, batch, deal, action, onClose, onSuccess }) {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      switch (action) {
        case 'approve':
          await sb.from('batches').update({ status: 'paid' }).eq('id', batch.id);
          break;
        case 'reject':
        ...
        case 'collect':
          ...
          // ✅ SEND EMAIL TO CLIENT - BATCH READY FOR PICKUP
          try {
            console.log('📧 Sending batch ready email...');
            if (deal?.client_id) {
              ...
              if (clientData?.email) {
                const warehouse = deal.warehouse || 'Main Warehouse';
                await sendBatchReadyEmail(
                  clientData.email,
                ...
                console.log('✅ Batch ready email sent');
              }
            }
          } catch (emailErr) {
